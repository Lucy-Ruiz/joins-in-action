...
#Total amount of all records from the first table = 76 entries returned
SELECT * FROM movies LEFT JOIN users on favorite_movie_id = movie_id;
...
SELECT COUNT(*) AS popular, lead_studio 
FROM movies INNER JOIN users on favorite_movie_id = movie_id
...
SELECT AVG(rotten_tomatoes) AS rotten_tomatoes_fav
FROM movies INNER JOIN users on favorite_movie_id = movie_id;


-- <<<<<<<<<<<<<<<<<<<<<< PROBLEM 6 >>>>>>>>>>>>>>>>>>>>>>>
-- Determine which two movies have duplicated entries in the dataset. (HINT: No join is needed for this problem)
-- EXPECTED RESULT: Mamma Mia!, Gnomeo and Juliet
-- It is important to be able to identify dirty data within a dataset!
SELECT COUNT(*) AS duplicated, film
FROM movies
GROUP BY film
